...
class ScenarioMammasoftExtPriorMissingModality(ScenarioBase):
    SCENARIO_NAME = "mammasoft_ext_prior_missing_modality"
    ...
    def create_external_prior_config(self, study_index=1):
        ...
        for i, vp in enumerate(views):
            ...
            tags["StudyDescription"] += " - External Prior (Missing Modality)"
            tags["SeriesDescription"] += " - No Modality"
            tags["InstanceNumber"] = i + 1
            tags["CustomFileName"] = f"MammasoftExtNoMod_Study{study_index}_{vp}_{i+1}.dcm"
...
if __name__ == "__main__":
    scenario = ScenarioMammasoftExtPriorMissingModality()
