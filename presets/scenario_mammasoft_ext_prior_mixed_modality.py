#!/usr/bin/env python3
"""
Generate two external prior studies for the same Mammasoft patient:
  - The first uses MG views (RCC, LCC, RMLO, LMLO).
  - The second uses MR1..MR4 with Modality = "MR".
  - Invert the patient name.
"""

import random
import json
from presets.scenario_base import (
    ScenarioBase,
    random_uid,
    random_six_digit,
    random_date_yyyymmdd
)

def invert_name(name):
    parts = name.split("^")
    if len(parts) == 2:
        return f"{parts[1]}^{parts[0]}"
    return name

class ScenarioMammasoftExtPriorMixedModality(ScenarioBase):
    SCENARIO_NAME = "mammasoft_ext_prior_mixed_modality"

    STATIC_PATIENT_NAME = "TC303A^Klientin-1"
    STATIC_PATIENT_BIRTHDATE = "19630330"

    def __init__(self):
        super().__init__()
        self.patient_name = invert_name(self.STATIC_PATIENT_NAME)
        self.birth_date_yyyymmdd = self.STATIC_PATIENT_BIRTHDATE

    def create_mg_prior(self):
        study_uid = random_uid()
        study_id = random_six_digit()
        accession = f"{random_six_digit()}-{random.randint(1000, 9999)}"
        external_date = random_date_yyyymmdd(2005, 2015)

        views = ["RCC", "LCC", "RMLO", "LMLO"]
        images = []
        for i, vp in enumerate(views):
            tags = self.make_mg_image_tags(
                view_position=vp,
                is_cancerous=False,
                study_uid=study_uid,
                study_date=external_date,
                study_id=study_id,
                accession=accession
            )
            tags["Modality"] = "MG"
            tags["StudyDescription"] += " - External Prior (MG)"
            tags["SeriesDescription"] += " - MixedMod"
            tags["InstanceNumber"] = i + 1
            tags["CustomFileName"] = f"MammasoftExtMG_{vp}_{i+1}.dcm"
            images.append({"tags": tags})

        return {"cancerous": False, "count": len(images), "images": images}

    def create_mr_prior(self):
        study_uid = random_uid()
        study_id = random_six_digit()
        accession = f"{random_six_digit()}-{random.randint(1000, 9999)}"
        external_date = random_date_yyyymmdd(2005, 2015)

        views = ["MR1", "MR2", "MR3", "MR4"]
        images = []
        for i, vp in enumerate(views):
            tags = self.make_mg_image_tags(
                view_position=vp,
                is_cancerous=False,
                study_uid=study_uid,
                study_date=external_date,
                study_id=study_id,
                accession=accession
            )
            tags["Modality"] = "MR"
            tags["StudyDescription"] += " - External Prior (MR)"
            tags["SeriesDescription"] += " - MixedMod"
            tags["InstanceNumber"] = i + 1
            tags["CustomFileName"] = f"MammasoftExtMR_{vp}_{i+1}.dcm"
            images.append({"tags": tags})

        return {"cancerous": False, "count": len(images), "images": images}

    def build_scenario_config(self):
        mg_study = self.create_mg_prior()
        mr_study = self.create_mr_prior()
        return {
            "current_study": {"cancerous": False, "count": 0, "images": []},
            "internal_priors": [],
            "external_priors": [mg_study, mr_study]
        }

if __name__ == "__main__":
    scenario = ScenarioMammasoftExtPriorMixedModality()
    config = scenario.build_scenario_config()
    print(json.dumps(config, indent=2))
